...
async def on_disconnect():
    ...
    # If there are active recordings, don't clean up - let them continue
    if connections:
        logger.info("🔄 Active recordings detected, skipping cleanup to preserve audio")
        return
    
    # Only clean up if no active recordings
    logger.info("🧹 No active recordings, cleaning up state")
    connections.clear()
    parts.clear()
